defmodule Input do
  ...
  # attr :variant, :string, default: "solid", values: ["solid", "soft"]
  def input(%{type: "switch", value: value} = assigns) do
    ...
    <div class="relative inline-block">
      <input
        ...
        class={"peer relative p-px bg-gray-100 text-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:ring-blue-600 disabled:opacity-50 disabled:pointer-events-none checked:bg-none dark:bg-neutral-800 dark:border-neutral-700 dark:focus:ring-offset-gray-600

        before:inline-block before:bg-white before:translate-x-0 checked:before:translate-x-full before:rounded-full before:transform before:ring-0 before:transition before:ease-in-out before:duration-200 dark:before:bg-neutral-400 #{size(@size)} #{variant(@variant)} #{@class}"}
        {@rest}
      />
  ...
  def input(%{variant: "default"} = assigns) do
    ...
    <div>
      ...
      <div class="flex rounded-lg w-full">
        ...
        <div class="relative w-full">
          <input
           ...
           dark:focus:ring-neutral-600 dark:border-neutral-700 border-gray-200 focus:border-blue-500 focus:ring-blue-500 #{@class}",
              ...
              @errors == [] && @used && "border-teal-500 focus:border-teal-500 focus:ring-teal-500",
              @icon != "hero-" && "sm:ps-11",
          ...
          <div class="hidden absolute inset-y-0 start-0 sm:flex items-center pointer-events-none z-20 ps-4">
            <.icon
              name={@icon}
              class="shrink-0 h-4 w-4 text-gray-400 dark:text-neutral-600"
            />
          </div>
          <div
            :if={@errors != []}
        ...
        <.link
          :if={@suffix}
          patch={@suffix.navigate}
          class="px-1 shrink-0 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-e-md border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none"
        >
          <.icon
            :if={Map.has_key?(@suffix, :icon)}
            name={@suffix.icon}
            class="shrink-0 h-4 w-4"
          /> <%= if Map.has_key?(@suffix, :label), do: @suffix.label %>
        </.link>
      </div>
  ...
  def input(%{variant: "gray"} = assigns) do
    ...
    <div>
      <div class="flex justify-between">
        ...
        <span :if={@hint} class="block mb-2 text-sm text-gray-500 dark:text-neutral-500">
          <%= @hint %>
        </span>
      </div>
      <div class="relative">
        <input
          ...
          class={[
            ...
            @icon && "ps-11",
            @errors == [] && @used && "bg-teal-100/50",
            @errors != [] && "bg-red-100/50"
          ]}
          {@rest}
        />
        <div
        ...
        >
          <.icon class="text-gray-500 dark:text-neutral-500" name={@icon} />
        </div>
        <div
          :if={@errors != []}
          class="absolute inset-y-0 end-0 flex items-center pointer-events-none pe-3"
      ...
      <p :if={@helper} class="mt-2 text-sm text-gray-500 dark:text-neutral-500" id="input-helper-text">
        <%= @helper %>
      </p>
      <.error :for={msg <- @errors}>
        <%= msg %>
    ...
    <div class="relative">
      <div class="relative">
        ...
        <div class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-2 peer-disabled:opacity-50 peer-disabled:pointer-events-none">
          <.icon
            class="shrink-0 h-4 w-4 text-gray-500 dark:text-neutral-500"
            name={@icon}
          />
        </div>
        <div
  ...
  defp size("xs"), do: "w-[35px] h-[21px] before:size-4"

  defp size("sm"), do: "w-11 h-6 before:size-5"

  defp size("md"), do: "w-[3.25rem] h-7 before:size-6"
  defp size("lg"), do: "w-[4.25rem] h-9 before:h-8 before:w-8"
 defp variant("solid"), do: "shrink-0 border-transparent checked:text-blue-600 checked:border-blue-600 focus:checked:border-blue-600 dark:checked:bg-blue-500 dark:checked:border-blue-500 checked:before:bg-blue-200 dark:checked:before:bg-blue-200"

 defp variant("soft"), do: "border border-gray-200 checked:text-blue-100 checked:border-blue-200 focus:checked:border-blue-200 dark:checked:bg-blue-800/30 dark:checked:border-blue-800 checked:before:bg-blue-600 dark:checked:before:bg-blue-500"
end
